...
from helper.recipeExporterRepository import RecipeExporterRepository
...
class Recipe(Element):
    ...
    def __repr__(self):
        return (
            'recipe[name="%s", brewer="%s", type=%s, volume=%s, efficiency=%s,'
            'boil=%s, recipeNotes="%s", style="%s"]' % (
                self.name, self.brewer, self.type, self.volume,
                self.efficiency, self.boil, self.recipeNotes, self.style
            )
        )
    ...
    def compute_equivSugar(self):
        ...
        for f in self.listeFermentables:
            ...
            if f.type in (model.constants.FERMENTABLE_TYPE_EXTRACT,
                          model.constants.FERMENTABLE_TYPE_DRY_EXTRACT,
                          model.constants.FERMENTABLE_TYPE_SUGAR):
                self.equivSugarNonMashed += f.equivSucre()
            else:
                self.equivSugarMashed += f.equivSucre()

            # on refait la même chose pour déterminer la densité pré-ébullition
            # en cas d'addition tardive de sucre, ce qui influence le calcul
            # des IBUs.
            if f.useAfterBoil is False:
    ...
    def compute_GU(self):
        ...
        return (
            (383.89 * self.equivSugarMashed / float(self.volume)) *
            (self.efficiency / 100) +
            (383.89 * self.equivSugarNonMashed / float(self.volume))
        )

    def compute_GU_PreBoil(self):
        self.compute_equivSugar()
        return (
            (383.89 * self.equivSugarMashedPreBoil / float(self.volume)) *
            (self.efficiency / 100) +
            (383.89 * self.equivSugarNonMashedPreBoil / float(self.volume))
        )

    def compute_FG(self):
        # calcul de la FG. Si il y a plusieurs levures, on recupere
        # l'attenuation la plus elevee.
        levureAttenDec = sorted(
            self.listeYeasts, reverse=True,
            key=lambda levure: levure.attenuation
        )
    ...
    def compute_IBUPart(self):
        ...
        for h in self.listeHops:
            ...
            try:
                if h.use not in (model.constants.HOP_USE_DRY_HOP,
                                 model.constants.HOP_USE_AROMA):
    ...
    def compute_grainWeight(self):
        lst = [
            f.amount for f in self.listeFermentables if f.type not in
            (model.constants.FERMENTABLE_TYPE_EXTRACT,
             model.constants.FERMENTABLE_TYPE_DRY_EXTRACT,
             model.constants.FERMENTABLE_TYPE_SUGAR)
        ]
        return sum(lst)
