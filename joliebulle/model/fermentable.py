...
class Fermentable(Element):
    ...
    def __repr__(self):
        return (
            'fermentable[name="%s", amount=%s, type=%s, yield=%s, '
            'recommendMash=%s, color=%s, useAfterBoil=%s]' % (
                self.name, self.amount, self.type, self.fyield,
                self.recommendMash, self.color, self.useAfterBoil
            )
        )

    def equivSucre(self):
        # division par 1000 et 100 pour passer des g aux kg et parce que le
        # rendement est un pourcentage
